		...
		1)
			...
			while true; do
				...
				if ss -tuln | grep -q ":$app_port "; then
					echo -e "${gl_hong}실수:${gl_bai}포트$app_port 已被占用，请更换一个端口"
...
dd_xitong() {
		...
		dd_xitong_2() {
		  echo -e "재설치 후 초기 사용자 이름:${gl_huang}Administrator${gl_bai}  初始密码: ${gl_huang}Teddysun.com${gl_bai}초기 포트:${gl_huang}3389${gl_bai}"
		  echo -e "계속하려면 아무 키나 누르세요..."
		  read -n 1 -s -r -p ""
		  install wget
		  dd_xitong_MollyLau
...
EOF
		  ...
		  66)
			  ...
			  echo -e "9. DNS 주소 자동 최적화${gl_huang}해외: 1.1.1.1 8.8.8.8 국내: 223.5.5.5${gl_bai}"
		  	  echo -e "10. 设置网络为${gl_huang}IPv4 우선순위${gl_bai}"
		...
		case "$Limiting" in
		   ...
		   24) # 复制文件目录
			 ...
			 25) # 传送文件至远端服务器
				...
				kj_ssh_read_host_user_port "请输入远端服务器IP: " "원격 서버 사용자 이름(기본 루트)을 입력하십시오:" "로그인 포트(기본값 22)를 입력하세요." "root" "22"
				local remote_ip="$KJ_SSH_HOST"
